...
import { Trash2, User, BookOpen, CalendarDays, Undo2 } from "lucide-react";

export default function Trash({ items, onRestore, onWipe }) {
  if (items.length === 0) {
    return (
    ...
    <div className="grid grid-cols-1 gap-6">
      {items.map((r) => {
        ...
        return (
          <div
            key={r.id}
            className="relative border border-slate-200 rounded-sm shadow-sm transition-all duration-300 overflow-hidden hover:shadow-md bg-white group opacity-75 hover:opacity-100"
          >
            <div className="grid grid-cols-1 md:grid-cols-[140px_1fr_140px] h-full">
              ...
              <div className="p-6 flex flex-col justify-center">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="text-xl font-black text-slate-600 line-through decoration-slate-300 leading-tight">
              ...
              <div className="p-4 flex flex-col gap-2 border-t md:border-t-0 md:border-l border-slate-100 bg-slate-50/50 justify-center">
                ...
                <button
                  onClick={() => onWipe(r)}
                  className="w-full py-2 bg-white border border-slate-200 text-slate-400 rounded-sm text-[10px] font-black uppercase tracking-widest hover:border-red-600 hover:text-white hover:bg-red-600 transition-all flex items-center justify-center gap-2"
                >
                  <Trash2 size={14} /> Wipe
                </button>
              </div>
